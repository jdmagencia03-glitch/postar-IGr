  ...
  for (const batch of batches) {
    ...
    if (batch.status === "uploading" && failedCount >= 20) {
      errors.push({
        ...
        message: `${failedCount} de ${totalFiles} vídeos falharam — os pendentes continuam em segundo plano.`,
        probableCause: "Saturação de upload ou instabilidade prolongada.",
        recommendedAction: "Falhas não bloqueiam o lote. Use Recuperar upload se o envio parar.",
        accountId,
        platform,
        uploadBatchId: batch.id,
